...
function BronsteinTimer(game, time, bonus) {
	// Validation
	...
	// System
	this.system = {};
	this.system.name = "Bronstein";
	this.system.time = time;
	this.system.bonus = bonus
}

BronsteinTimer.prototype = {
	// Validation
	...
	pause: function(give_bonus) {
		if (this.status == ST_COUNTING) {
			...
			var elapsed = (this.last_pause - this.last_resume) / 1000;

			// We only need to change the time if the elasped time is greater than the bonus,
			// otherwise don't change the time at all.
			if (elapsed > bonus) {
